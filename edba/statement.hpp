...
public:
    ...
    statement& reset_bindings()
    {
        if (stmt_)
        {
            placeholder_ = 1;
            stmt_->reset_bindings();
        }
        return *this;
    }
    ...
    {
        if (!stmt_)
            throw empty_statement("last_insert_id");
    ...
    {
        if (!stmt_)
            throw empty_statement("sequence_last");
    ...
    {
        if (!stmt_) 
            throw empty_statement("first_row");
